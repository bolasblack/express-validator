    ...
    req.validationErrors = function(mapped) {
      if (mapped && req._validationErrors.length > 0) {
        ...
        req._validationErrors.forEach(function(err) {
          errors[err.param] = (errors[err.param] || err);
        });
...
function makeValidator(methodName, container) {
  return function() {
    var args = [], options, failMsg;
    args.push(this.value);
    args = args.concat(Array.prototype.slice.call(arguments));
    options = _.last(args);

    if (_.isPlainObject(options)) {
      failMsg = options.message;
    }
    failMsg = failMsg || this.failMsg || 'Invalid value';
    ...
    if (!isValid) {
      var error = this.errorFormatter(this.param, failMsg, this.value);
...
module.exports.ValidatorChain = ValidatorChain;
module.exports.makeValidator = makeValidator;
module.exports.makeSanitizer = makeSanitizer;
